...
def tf_idf_documents_given(stories):
	...
	# for story in stories:
	# 	documents.append(story["content"])
	for key, value in stories.iteritems():
...
def tf_idf_selector(idf_group, collection):
	...
	if idf_group == "A":
		...
		for topic in all_topics:
			...
			#combine all stories under the same agency together
			stories_merged_by_source = {}
			for result in db_results:
				if result['source'] not in stories_merged_by_source:
					stories_merged_by_source[result['source']] = ""

				stories_merged_by_source[result['source']] += " " + result['content']

			# pprint (stories_merged_by_source)

			# sys.exit()

			# Call the tf_idf function to compute given the current selection
			# tf.append(tf_idf([item for item in db_results]))
			tf.append(tf_idf_documents_given(stories_merged_by_source))
...
if __name__ == "__main__":
	...
	for agency, alignment in pol_align_lookup.iteritems():
		...
		# counting number of features being fed

		# for test_item in X_test:
		# 	print(np.count_nonzero(test_item))
		# 	print(test_item.size)
		# sys.exit()
        #
		# pprint (X_test)
		# print "count is: " + str(np.count_nonzero(X_test))
		# print "Sum is: " + str(np.sum(X_test))
		# pprint(y_test)
		# sys.exit()
		...
		print('Linear score: ' + str(linear_regres.predict(X_test)))
